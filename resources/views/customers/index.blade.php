                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('customers.show', $customer) }}"
                                               ...
                                               title="{{ __('customers.actions.view') }}">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @can('customers.edit')
                                                <a href="{{ route('customers.edit', $customer) }}"
                                                   class="btn btn-outline-warning"
                                                   title="{{ __('customers.actions.edit') }}">
                                            ...
                                            @can('customers.delete')
                                                <form action="{{ route('customers.destroy', $customer) }}"
                                                      method="POST"
                                                      class="d-inline"
                                                      onsubmit="return confirm('{{ __('customers.messages.confirm_delete') }}')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                            class="btn btn-outline-danger"
                                                            title="{{ __('customers.actions.delete') }}"
                                                            {{ $customer->orders_count > 0 ? 'disabled' : '' }}>
